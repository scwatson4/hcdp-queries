...
| **Daily summary** | `mv_daily_station_summary_qc` | Daily | 2022 → present | 69k+ | Mirrors mesonet |
| **Monthly summary** | `mv_monthly_station_summary_qc` | Monthly | 2022 → present | 2.4k+ | Mirrors mesonet |
...
**`_qc` vs `_raw`:** Always use the `_qc` views (`mv_daily_station_summary_qc`, `mv_monthly_station_summary_qc`, `v_mesonet_measurements_qc`) unless specifically investigating data quality. The `_raw` versions contain sentinel codes (7999, -9999), impossible temperatures (-175°C), and inflated soil moisture that will contaminate any aggregate.
